...
class MLActionDecider: #{{{
   ...
   def scores(self, conf):  ## TODO: who uses this??
      ...
      action,scores=self.m.predict(fs)
      scores = dict(enumerate(scores))  # [-122, 0.3, 3] -> {0:-122, 1:0.3, 2:3} 
      if conf.i >= len(conf.sent): del scores[SHIFT]
      return scores
   ...
   def next_action(self, stack, deps, sent, i):
      ...
      if action == REDUCE_R:
         if stack[-1]['parent']==stack[-2]['id']:
            if len(self.childs_of_x[stack[-1]['id']] - self.connected_childs) > 0:
               print "R not connecting",_ids(stack[-1]),_ids(stack[-2]),"because:",_enrich(self.childs_of_x[stack[-1]['id']] - self.connected_childs)
            else:
               print "R not XX"


      if action==REDUCE_L:
         if len(self.childs_of_x[stack[-2]['id']] - self.connected_childs) < 1:
            self.connected_childs.add(stack[-2]['id'])
      if action==REDUCE_R:
         if len(self.childs_of_x[stack[-1]['id']] - self.connected_childs) < 1:
...
class MLTrainerWrongActionException(Exception): pass
...
class MLTrainerActionDecider: #{{{
   ...
   def next_action(self, stack, deps, sent, i, conf=None):
      action   = self.decider.next_action(stack, deps, sent, i)
      mlaction = self.ml.update(action, self.fs.extract(stack, deps, sent, i))
      if action!=mlaction:
         if self.earlyUpdate:
...
class MLPassiveAggressiveTrainerActionDecider: #{{{
   ...
   def next_action(self, stack, deps, sent, i):
      action   = self.decider.next_action(stack, deps, sent, i)
      mlaction = self.ml.do_pa_update(self.fs.extract(stack, deps, sent, i),action,C=1.0)
      if action!=mlaction:
         if self.earlyUpdate:
            raise MLTrainerWrongActionException()
      return action
...
class MLTrainerActionDecider2: #{{{
   ...
   def score_deps(self,deps, sent):
      ...
      sent_deps = set()
      for tok in sent:
         if tok['id']==0: continue
         sent_deps.add((tok['parent'],tok['id']))
      for pc in sent_deps:
         if pc not in deps: 
            score+=0.2
      for pc in deps:
         if pc not in sent_deps: score+=1
      return score
      ...
      while not newconf.is_in_finish_state():
         try:
            if ml:
               ...
               next = decider.next_action(newconf.stack,newconf.deps,newconf.sent,newconf.i)
   ...
   def next_ml_action(self, conf):
      ...
      act_scores = [(score,act) for act,score in self.ml.get_scores(features).items() if act in conf.valid_actions()]
   ...
   def next_action(self, stack, deps, sent, i,conf=None):
      ...
      if pred_a != goldaction:
         ...
         update_cost = self.cum_score_of_action(self.next_ml_action(conf), conf, ml=True)
         if noupdate_cost < update_cost:
            print "noupdate:",noupdate_cost, "update:", update_cost
            # undo prev update
            self.ml.add(features, goldaction, -1.0)
            self.ml.add(features, pred_a, 1.0)
      return goldaction
   ...
   def cum_score_of_action(self, action, conf, ml=False):
      ...
      while not newconf.is_in_finish_state():
         try:
            ...
            #print "oracle says",next,"but it is illegal, probably at end"
            assert(len(newconf.sent)==newconf.i)
            break
      return self.score_deps(newconf.deps, newconf.sent)
   ...
   def next_action(self, stack, deps, sent, i,conf=None):
      ...
      for score,act in act_scores:
         if pred_a != act:
            ...
            self.ml.add(features, pred_a, -1.0)
            costs.append((self.cum_score_of_action(act, conf,ml=True),act))
            self.ml.add(features, act, -1.0)
            self.ml.add(features, pred_a, 1.0)
         else:
      ...
      if act != pred_a:
         #print min_cost,noupdate_cost,act,goldaction
         self.ml.add(features, act, 1.0)
         self.ml.add(features, pred_a, -1.0)
      else:
         pass
   ...
   def next_action(self, stack, deps, sent, i):
      ...
      if stack and sent[i:] and stack[-1]['id']==sent[i]['parent']:
         if deps.has_parent(sent[i]):
            #print "skipping add",stack[-1],sent[i]
            pass
         else:
...
class ArcStandardConfiguration(Configuration): #{{{
   ...
   def valid_actions(self):
      res=[]
      if self.sent[self.i:]: res.append(SHIFT)
      if len(self.stack) >= 2: 
         res.append(REDUCE_L)
         res.append(REDUCE_R)
      return res
...
class Old_ArcEagerConfiguration(Configuration): #{{{
   ...
   def ABC(self):
      ...
      if len(stack) == 0: res.remove(POP)
      elif not self.deps.has_parent(stack[-1]): res.remove(POP)
      ...
      if len(self.stack) < 1: 
         ...
         if self.deps.has_parent(self.stack[-1]): res.remove(REDUCE_L)
...
class ArcEagerConfiguration(Configuration): #{{{
   ...
   def valid_actions(self):
      ...
      elif not self.deps.has_parent(self.stack[-1]): res.remove(POP)
...
class TransitionBasedParser: #{{{
   ...
   def decide(self, conf):
      actions = self.d.next_actions(conf.stack, conf.deps, conf.sent, conf.i, conf)
      return actions
...
class ErrorInspectionParser(ArcStandardParser2): #{{{
   ...
   def decide(self, conf):
      ...
      real   = self.oracle.next_action(conf.stack, conf.deps, conf.sent, conf.i)
      if action != real:
